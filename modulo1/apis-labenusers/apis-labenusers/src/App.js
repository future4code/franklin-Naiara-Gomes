...
const App = () => {
  ...
  const criaUserslist = () => {
    ...
    const body = {
      name: inputUserslist, valorInputEmail
    }
  ...
  return (
    <div className="App">
      ...
      <br/>
      <input value={inputUserslist} onChange={handleInputNome} placeholder={"Nome"}></input>
      <input value={valorInputEmail} onChange={handleInputEmail} placeholder={"Email"}></input>
      
      <button onClick={criaUserslist}>Cadastrar usuário</button>
      {userslists.map((userslist) => {
        return (
          <li key={userslist.id}>{userslist.name}</li>
